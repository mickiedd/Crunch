...
# launch_game.sh
# Launch the Unreal Editor with the project in the same folder as this script.
# Usage: ./launch_game.sh [path/to/Project.uproject]

# Resolve base directory (script location)
BASE_DIR="$(cd "$(dirname "$0")" && pwd)"

# Default uproject is Crunch.uproject next to this script, or first arg if provided
UPROJECT_PATH="${1:-$BASE_DIR/Crunch.uproject}"
